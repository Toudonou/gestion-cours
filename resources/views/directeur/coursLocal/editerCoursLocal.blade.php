...
<div class="card shadow mb-4" style="max-width: 800px; margin: auto;">
    <div class="card-body">
        <div class="table-responsive">
            <h2 class="center-text mb-4">Editer un cours local</h2>
            <form action="{{route('editer-cours-local', ['idDirecteur' => $directeur->id, 'idCours' => $cours->id])}}" method="post">
                @csrf
                ...
                <select class="form-control form-control-user" id="enseignant" name="enseignant" style="margin-bottom: 20px;" value="{{old('enseignant')}}" required>
                    <option value="{{$info->enseignant_local_id}}" selected>{{$info->nom}} {{$info->prenom}}</option>
